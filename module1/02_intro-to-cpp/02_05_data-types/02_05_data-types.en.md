...
In C++, everything has a type. You can think of a type like a label. The compiler uses these types to organize memory and use the appropriate CPU instructions when the machine code is produced.
...
Integers are fundamental data types used to store numeric whole numbers.

> Integers are whole numbers. `-1`, `0`, `1`, and `2` are examples of integers. Numbers such as `24.7` or `357.76` are **not** integers. (These are **floating-point** numbers, and C++ can use them, too. We will learn this later.)

Signed integers can store both positive and negative numbers, including zero. This can be useful for storing values such as a joystick position, coordinates, or directions, all of which can have negative values.

Unsigned integers can only store positive numbers and zero. This can be useful when dealing with quantities that can never be negative, such as sizes, lengths, or counts, or when doing bit-level arithmetic.
...
2. `int`: This is the most commonly used integer type. It's typically used for numeric operations and loop counters. For example, you might use an `int` to store a score in a game.

    ```cpp
    int points { 60'000 };
    ```

3. `long`: This type is often used when the range of values that an `int` can hold is insufficient. For example, it can be used to store the human population of Earth.

    ```cpp
    long earthPopulation { 7'888'000'000 };
    ```
...
4. `long long`: This type is used when the range of values that a `long` can hold is insufficient. It can handle very large numbers. For instance, you could use a `long long` to store the number of stars in the universe.
    ...
    ```cpp
    long long starsInUniverse { 1'000'000'000'000'000'000 };
    ```
...
unsigned long lightSpeed = 299'792'458;
```
...
> Remember that the exact range of values that can be represented by these types can vary depending on the system and compiler. For example, an `int` on a 32-bit system might be different than an `int` on a 64-bit system.
...
`float` numbers have limits to their accuracy and precision. `float` numbers are only accurate until around 5-7 decimals places. If you need more precise and accurate numbers, for example when doing scientific or high-precision mathematics calculations, you should use a `double`.
...
The `char` keyword is used to declare character type variables. For example:
...
This approach is lower-level and doesn't offer the flexibility and functionality of the `std::string` class that's used in modern C++. This class, which is part of the C++ Standard Library, provides a higher-level, easier-to-use way to work with strings.
...
> Functions provided by a class are called **member functions**, but classes will be covered in more depth in the next module.
...
You can work with strings in C++ at a low level by dealing with them as arrays of characters. However, in most cases, it's much easier to use the `std::string` class provided by the C++ Standard Library.
...
`isXLessThanY` will evaluate to `true` because `x < y` evaluates to `5 < 10`, which is true. 
...
`void` is a special data type that represents the absence of type. It essentially means "no type". In C++, it's most commonly used as a function return type.

When `void` is used as the return type for a function, it indicates that the function does not return a value. For example:
...
Let's practice. How would you declare an array of `long` integers and initialize it with three values?
...
Each value in an array has an **index**. The index of a value is a number that represents its position in the array. In C++ 0 is the first index of an array.
